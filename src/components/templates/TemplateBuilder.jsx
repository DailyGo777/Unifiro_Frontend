...
export default function TemplateBuilder({
  ...
  const handleDrop = (e) => {
    ...
    const newItem = {
      ...
      y: Math.max(0, y - 40),
      w:
        rendererType === "hero" || rendererType === "section"
          ? 800
          : rendererType === "textarea" || rendererType === "form-group"
            ? 350
            : 300,
      h:
        rendererType === "hero"
  ...
  const getDefaultContent = (type, label) => {
    ...
    if (type === "header")
      return { title: "Header", subtitle: "Subtitle", color: "transparent" };
    if (type === "navigation")
      return { title: "Brand", links: ["Home", "Link"], color: "#ffffff" };
    if (label === "Hero Section" || type === "hero")
      return {
        title: "Hero Title",
    ...
    if (type === "forms" || label === "Forms")
      return { title: "Newsletter", label: "Email", color: "#ffffff" };
    if (type === "interactive" || label === "Interactive")
      return { icon: "map", label: "Feature", text: "Details here" };
    if (type === "social" || label === "Social")
      return { platforms: ["twitter", "instagram", "facebook"] };
    if (type === "utility" || label === "Utility")
  ...
  const startMoving = (e, id) => {
    ...
    const offsetX = startMouseX - item.x;
    const offsetY = startMouseY - item.y;
    ...
    const onMouseMove = (moveEvent) => {
      ...
      // ⛔ Do NOT start dragging until threshold crossed
  ...
  const loadTemplate = (items) => {
    if (
      window.confirm("Load new template? This will replace your current work.")
    ) {
      setCanvasItems(
        items.map((i) => ({ ...i, id: Date.now() + Math.random().toString() })),
      );
    }
  };
  ...
  const handlePublish = () => {
    ...
    try {
      ...
      if (initialId) {
        // Update existing
        const existing =
          personalTemplates.find((t) => t.id === initialId) || {};
        templateToSave = {
          ...existing,
          title: templateName,
          items: canvasItems,
        ...
        updatedTemplates = personalTemplates.map((t) =>
          t.id === initialId ? templateToSave : t,
        );
      } else {
        // Create new
        templateToSave = {
          ...
          color: "bg-white",
          image:
            canvasItems.find((i) => i.type === "image")?.properties?.src ||
            null,
          items: canvasItems,
      ...
      if (e.name === "QuotaExceededError" || e.code === 22) {
        toast.error(
          "Storage full! Please delete some old templates or images.",
        );
      } else {
  ...
  if (viewMode === "preview") {
    return (
      <div className="fixed inset-0 bg-white z-[100] overflow-auto flex flex-col items-center">
        ...
        >
          <div
            className="relative w-[1000px] h-full shadow-2xl bg-white shrink-0 origin-top scale-90 mt-10"
            style={{ backgroundColor: pageProperties.backgroundColor }}
          >
  ...
  return (
    <div className="flex h-screen w-full bg-white overflow-hidden font-sans text-slate-900">
      <div
        className={`transition-all duration-300 ease-in-out ${isSidebarOpen ? "w-fit" : "w-0 overflow-hidden opacity-0"}`}
      >
        ...
        <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-6 shrink-0 z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-1.5 flex items-center gap-2 text-sm font-semibold text-gray-600 hover:bg-gray-50 rounded-lg border border-gray-200 transition-all"
            >
              <Eye size={16} /> Preview
            </button>
            {isPublished ? (
              ...
              <button
                ...
                className={`px-6 py-1.5 flex items-center gap-2 text-sm font-bold text-white rounded-lg shadow-lg shadow-indigo-200 transition-all ${isSaving ? "bg-indigo-400" : "bg-indigo-600 hover:bg-indigo-700 hover:-translate-y-0.5"}`}
              >
        ...
        >
          <div
            ...
            style={{
              ...
              backgroundImage: pageProperties.gridVisible
              ...
              canvasItems.map((item) => (
                <div
                  ...
                  className={`absolute cursor-move transition-shadow ${
                    selectedId === item.id
                      ? "ring-[2.5px] ring-indigo-500 z-50"
                ...
                >
                  {selectedId === item.id && (
                    <>
                      <div className="absolute -top-12 left-0 flex items-center bg-white border border-gray-200 shadow-xl rounded-lg overflow-hidden z-50 p-1">
                        <div className="px-3 py-1 text-[10px] font-black text-indigo-600 uppercase tracking-widest">
                          {item.label}
                        </div>
                      ...
                      <ResizeHandle
                        position="top-left"
                        onMouseDown={(e) =>
                          startResizing(e, item.id, "top-left")
                        }
                      />
                      <ResizeHandle
                        position="top-right"
                        onMouseDown={(e) =>
                          startResizing(e, item.id, "top-right")
                        }
                      />
                      <ResizeHandle
                        position="bottom-left"
                        onMouseDown={(e) =>
                          startResizing(e, item.id, "bottom-left")
                        }
                      />
                      <ResizeHandle
                        position="bottom-right"
                        onMouseDown={(e) =>
                          startResizing(e, item.id, "bottom-right")
                        }
                      />
                    </>
                  )}
                  <Renderer
                    ...
                    onUpdate={(props) =>
                      updateItem(item.id, {
                        properties: { ...item.properties, ...props },
                      })
                    }
                  />
                </div>
      ...
      <aside
        className={`bg-white border-l border-gray-200 flex flex-col z-30 shrink-0 shadow-xl transition-all duration-300 ease-in-out ${selectedItem || isPageSettingsOpen ? "w-[320px] translate-x-0" : "w-0 translate-x-10 overflow-hidden opacity-0"}`}
      >
        {selectedItem ? (
          <div className="flex flex-col h-full animate-in slide-in-from-right-4 duration-200">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              ...
              <button
                onClick={() => setSelectedId(null)}
                className="p-2 hover:bg-white rounded-xl shadow-sm border border-gray-100 text-gray-400 transition-all"
              >
                <X size={16} />
              </button>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    >
                      ...
                      <input
                        type="number"
                        value={Math.round(selectedItem[f.k] || 0)}
                        onChange={(e) =>
                      ...
                      ([key, value]) => {
                        ...
                        return (
                          <div key={key} className="space-y-1">
                            <span className="text-[10px] font-bold text-gray-500 ml-1 capitalize">
                              {key}
                            </span>
                            <input
                              type="text"
                              ...
                              className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium focus:ring-2 focus:ring-indigo-500/10 outline-none transition-all shadow-sm"
                            />
                          </div>
                        );
                      },
            ...
            <div className="p-6 border-t border-gray-100">
              <button
                ...
                className="w-full py-3 bg-red-50 text-red-600 text-xs font-black rounded-xl hover:bg-red-100 transition-all uppercase tracking-widest"
              >
                Delete Component
              </button>
            </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 bg-white border border-gray-200 p-2 rounded-xl shadow-sm">
                  <input
                    ...
                    className="w-8 h-8 rounded cursor-pointer"
                  />
                  <span className="text-xs font-mono font-bold text-gray-400 uppercase flex-1">
                    {pageProperties.backgroundColor}
                  </span>
                </div>
              </div>
