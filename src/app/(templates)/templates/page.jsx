"use client";

import React, { useState, useEffect } from 'react';
import Image from "next/image";
import Link from "next/link";
import { Plus, ArrowLeft, X, FolderOpen, Palette } from "lucide-react";
import { kanLayout, standardLayout } from "@/data/templates/layouts";
import TemplateLanding from "@/components/templates/TemplateLanding";
import CategoryFilter from "@/components/templates/CategoryFilter";
import TemplateCard from "@/components/templates/TemplateCard";
import TemplateBuilder from "@/components/templates/TemplateBuilder";

import { useTemplates } from "@/context/TemplatesContext";


const TemplatesPage = () => {
    const {
        view, setView,
        activeCategory, setActiveCategory,
        selectedLayout, setSelectedLayout,
        setIsComponentLibraryOpen
    } = useTemplates();

    const [personalTemplates, setPersonalTemplates] = useState([]);
    const [editingTemplate, setEditingTemplate] = useState(null);

    const categories = ["All", "Creators", "Product", "Marketing", "Personal"];
    const gradientClass = "bg-gradient-to-r from-orange-400 via-lime-400 to-green-400";

    const defaultTemplates = [
        { title: "Registration Form", category: "Personal", color: "bg-white", image: "https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&q=80&w=500" },
        { title: "Event Registration", category: "Marketing", color: "bg-purple-100", image: "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&q=80&w=500" },
        { title: "Party RSVP Form", category: "Personal", color: "bg-red-50", image: "https://images.unsplash.com/photo-1530103862676-de3c9da59af7?auto=format&fit=crop&q=80&w=500" },
        { title: "Wedding Form", category: "Personal", color: "bg-orange-50", image: "https://images.unsplash.com/photo-1519225468063-e7296d955fa2?auto=format&fit=crop&q=80&w=500" },
    ];

    // Load personal templates from localStorage
    useEffect(() => {
        const saved = localStorage.getItem('personalTemplates');
        if (saved) {
            try {
                setPersonalTemplates(JSON.parse(saved));
            } catch (e) {
                console.error("Failed to load personal templates", e);
            }
        }
    }, [view]);

    const templates = [...personalTemplates, ...defaultTemplates];

    const filteredTemplates = activeCategory === "All"
        ? templates
        : templates.filter(t => t.category === activeCategory);

    const deleteTemplate = (id) => {
        if (confirm('Are you sure you want to delete this template?')) {
            const updated = personalTemplates.filter(t => t.id !== id);
            setPersonalTemplates(updated);
            localStorage.setItem('personalTemplates', JSON.stringify(updated));
        }
    };

    const handleSelectTemplate = (templateOrLayout) => {
        const layoutData = templateOrLayout.items || templateOrLayout;
        setSelectedLayout(layoutData);
        setEditingTemplate(templateOrLayout.items ? templateOrLayout : null);
        setView('builder');
        setIsComponentLibraryOpen(true);
    };

    return (
        <div className={`overflow-x-hidden ${view !== 'builder' ? 'p-6 md:p-8' : ''}`}>
            <div className="w-full mx-auto">

                {/* 1. LANDING VIEW */}
                {view === 'landing' && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 flex flex-col items-center">
                        <div className="text-center mb-6">
                            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 tracking-tight leading-tight">
                                Set up your event in a guided flow.
                            </h1>
                            <p className="text-gray-600 mt-6 text-xl max-w-2xl mx-auto">
                                Add event details, build or select a registration form, upload required files, and publish when ready.
                            </p>
                        </div>
                        {/* card1 */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-4xl pb-12 transition-all duration-700">
                            {/* Professional Template (Plus Icon Card) */}
                            <div onClick={() => handleSelectTemplate(kanLayout)} className="group cursor-pointer">
                                <div className="h-120 w-[300px] rounded-[24px] bg-gradient-to-br from-[#A0E8D9] via-[#85DABF] to-[#CDF4A4] p-1 shadow-lg transition-all duration-500 transform group-hover:-translate-y-3 group-hover:shadow-2xl relative overflow-hidden flex items-center justify-center">
                                    <div className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                                    <Plus className="w-24 h-24 text-white drop-shadow-md" strokeWidth={1.5} />
                                    <div className="absolute bottom-10 text-white font-bold text-xl opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0 duration-300">
                                        Start from Template
                                    </div>
                                </div>
                            </div>

                            {/* Standard Template */}
                            <div onClick={() => handleSelectTemplate(kanLayout)} className="h-120 w-[300px] rounded-[24px] bg-white shadow-md overflow-hidden relative group border border-gray-100 flex items-center justify-center cursor-pointer transition-all hover:-translate-y-3">
                                <div className="absolute inset-0">
                                    <Image
                                        src="/template/clocktemplate.png"
                                        alt="Standard Template"
                                        fill
                                        className="object-cover"
                                    />
                                </div>
                                <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
                                <div className="relative z-10 p-6 text-white text-center w-full bg-gradient-to-t from-black/60 to-transparent self-end">
                                    <span className="font-bold text-xl">Standard Template</span>
                                </div>
                            </div>
                            {/* card 3 */}
                            <div onClick={() => handleSelectTemplate(kanLayout)} className="h-120 w-[300px] rounded-[24px] bg-white shadow-md overflow-hidden relative group border border-gray-100 flex items-center justify-center cursor-pointer transition-all hover:-translate-y-3">
                                <div className="absolute inset-0">
                                    <Image
                                        src="/template/webinartemplate.png"
                                        alt="Standard Template"
                                        fill
                                        className="object-cover"
                                    />
                                </div>
                                <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
                                <div className="relative z-10 p-6 text-white text-center w-full bg-gradient-to-t from-black/60 to-transparent self-end">
                                    <span className="font-bold text-xl">Webinar Template</span>
                                </div>
                            </div>
                        </div>

                        {/* Centered Browse Gallery Button */}
                        <div className="w-full flex justify-center pb-24">
                            <button
                                onClick={() => setView('gallery')}
                                className="w-full max-w-md h-20 rounded-full bg-[#0055D4] shadow-lg overflow-hidden relative group hover:-translate-y-3 transition-all duration-500 flex flex-col items-center justify-center cursor-pointer"
                            >
                                <div className="relative z-10 p-6 text-white text-center w-full bg-gradient-to-t from-black/60 to-transparent flex items-center justify-center">
                                    <p className="font-bold text-lg text-white">Browse Gallery</p>
                                </div>
                            </button>
                        </div>
                    </div>
                )}

                {/* 2. GALLERY VIEW (Merged) */}
                {view === 'gallery' && (
                    <div className="animate-in fade-in zoom-in-95 duration-500">
                        <div className="flex items-center justify-between mb-0 mt-0">
                            <button
                                onClick={() => setView('landing')}
                                className="flex items-center gap-2 text-gray-500 hover:text-gray-900 transition-colors font-medium group"
                            >
                                <div className="p-2 rounded-full bg-white shadow-sm border border-gray-100 group-hover:bg-gray-50">
                                    <ArrowLeft className="w-5 h-5" />
                                </div>
                            </button>

                            <button
                                onClick={() => setView('landing')}
                                className="p-2 hover:bg-gray-200 rounded-full transition-colors"
                            >
                                <X className="w-6 h-6 text-gray-400" />
                            </button>
                        </div>

                        <div className="text-center mb-6">
                            <h2 className="text-3xl font-bold md:text-4xl text-gray-900 tracking-tight">
                                Explore form & survey templates
                            </h2>
                            <p className="text-gray-800 mt-3 max-w-xl mx-auto">
                                Explore, pick, and customize templates to your needs. <br />
                                Discover how to <span className="underline cursor-pointer">use templates</span>,{" "}
                                <span className="underline cursor-pointer">create your own</span> or{" "}
                                <span className="underline cursor-pointer">submit your template</span> to the gallery.
                            </p>
                        </div>

                        <CategoryFilter
                            categories={categories}
                            activeCategory={activeCategory}
                            onCategoryChange={setActiveCategory}
                            filteredTemplates={filteredTemplates}
                            onDelete={deleteTemplate}
                            personalTemplates={personalTemplates}
                            gradientClass={gradientClass}
                        />

                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 pb-24">
                            {filteredTemplates.map((template, idx) => (
                                <TemplateCard
                                    key={idx}
                                    template={template}
                                    onEdit={handleSelectTemplate}
                                    onDelete={deleteTemplate}
                                    isPersonal={template.category === "Personal" && personalTemplates.some((pt) => pt.id === template.id)}
                                />
                            ))}
                        </div>
                    </div>
                )}
                {/* 3. BUILDER VIEW */}
                {view === 'builder' && (
                    <TemplateBuilder
                        key={editingTemplate?.id || 'new-builder'}
                        onBack={() => { setView('landing'); setSelectedLayout(null); setEditingTemplate(null); }}
                        initialItems={selectedLayout}
                        initialName={editingTemplate?.title}
                        initialId={editingTemplate?.id}
                    />
                )}

                {/* 4. PERSONAL TEMPLATES VIEW (Placeholder) */}
                {view === 'personal' && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-3xl font-bold text-gray-900">My Templates</h2>
                                <p className="text-gray-500 mt-2">Manage your published forms and templates.</p>
                            </div>
                            <button onClick={() => { setView('builder'); setSelectedLayout(null); setEditingTemplate(null); }} className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition-all flex items-center gap-2">
                                <Plus size={20} /> Create New
                            </button>
                        </div>

                        {personalTemplates.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 pb-24">
                                {personalTemplates.map((template) => (
                                    <TemplateCard
                                        key={template.id}
                                        template={template}
                                        onEdit={handleSelectTemplate}
                                        onDelete={deleteTemplate}
                                        isPersonal={true}
                                    />
                                ))}
                            </div>
                        ) : (
                            <div className="text-center py-20 bg-white rounded-[32px] border border-dashed border-gray-200">
                                <FolderOpen size={64} className="mx-auto text-gray-200 mb-6" />
                                <p className="text-gray-500">You haven't saved any custom templates yet.</p>
                                <button onClick={() => setView('gallery')} className="mt-8 px-6 py-3 border border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition-all">
                                    Browse Gallery
                                </button>
                            </div>
                        )}
                    </div>
                )}

                {/* 5. THEMES VIEW (Placeholder) */}
                {view === 'themes' && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 text-center py-20">
                        <Palette size={64} className="mx-auto text-gray-200 mb-6" />
                        <h2 className="text-3xl font-bold text-gray-900">Global Themes</h2>
                        <p className="text-gray-500 mt-2">Configure global brand styles and color palettes.</p>
                    </div>
                )}
            </div>
        </div>
    );
};

export default function App() {
    return <TemplatesPage />;
}